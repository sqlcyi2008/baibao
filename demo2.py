...
def proc_ipv4_packet(frame):
    ...
    print '[IPv4]: hdr_len: %d, payload_len: %d, id: %d, ip_src: %s, ip_dst: %s, ip_proto: %s, flags: %s' % (
    dummy_hdrlen, iplen, id_, ipv4_itoa(ip_src), ipv4_itoa(ip_dst), hex(protocol), bin(flags).replace('0b', ''))
...
eth_proto_info = {
    ...
    0x8100: {'proto': '802.1Q', 'disec': None,
             'desc': 'VLAN-tagged frame (IEEE 802.1Q) and Shortest Path Bridging IEEE 802.1aq[8]'},
    ...
    0x88A8: {'proto': '', 'disec': None,
             'desc': 'Provider Bridging (IEEE 802.1ad) & Shortest Path Bridging IEEE 802.1aq[8]'},
    ...
    0x8902: {'proto': '802.1ag', 'disec': None,
             'desc': 'IEEE 802.1ag Connectivity Fault Management (CFM) Protocol / ITU-T Recommendation Y.1731 (OAM)'},
...
def proc_eth_frame(args):
    ...
    if l2_proc_func != None:
        if eth_type == 0x800:  # IPv4
            print "[ETH]: src_mac: %s, dst_mac: %s, eth_type: %s, Name: %s" % (
            ethmac_itoa(eth_header[0]), ethmac_itoa(eth_header[1]), hex(eth_type), short_name)
            l2_proc_func(frame)
    else:
        print "[%s] " % short_name
